...
src/
  ...
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu
...
- `ScoreLabel` — DXB-06: reusable HUD widget (`Phaser.GameObjects.Text` subclass) showing a `prefix` + numeric value anchored to a viewport corner, responsively sized/positioned. Not DX-Ball-specific — `MainScene` uses four instances (`Score:` top-left, `Best:` top-right, `Lives:` bottom-left since DXB-07, `Level:` bottom-right since DXB-08). Since DXB-07: anchor supports all four corners (`top-left` | `top-right` | `bottom-left` | `bottom-right`), not just the top two. Since DXB-13: bold typeface, dark stroke, drop shadow, and per-stat colors (Score white, Best gold, Lives mint, Level cyan).
...
- `SelectMenu` — DXB-14: reusable vertical option list (arrow keys, Space / Enter, click). `ModeSelectScene` is the first caller. Not DX-Ball-specific.
...
DXB-14 Game Modes — a pre-run `ModeSelectScene` chooses Classic, Time
Attack, or Endless. Classic is the unchanged level sequence (win on
the last level). Time Attack is a 90-second highest-score run that
ends at 0:00. Endless wraps the existing levels forever and gradually
speeds the ball up. Score, lives, powerups, and audio keep their
existing call sites; the HUD shows the active mode. See
`docs/progress/DXB-14.md` for full details. `npm run typecheck` and
`npm run build` both verified passing.

## Next Recommended Task

The core gameplay loop now spans a full level sequence plus the expanded
powerup roster, audio, advanced brick types, a visual refresh, and three
game modes (win, lose, score, best score, lives, levels, powerups, SFX,
brick types, readable presentation, Classic / Time Attack / Endless).
Good candidates going forward, none yet confirmed with a requester:
...
- A pause overlay; `SelectMenu` is now the first reusable menu widget.
  A visible mute toggle would fit naturally there.
