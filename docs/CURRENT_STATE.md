...
src/
  ...
  systems/    GameViewport, SceneKeys, HighScoreStore, AudioManager, ThemeStore
  entities/   dx-ball/ (Paddle, Ball, Brick, BrickType, BrickGrid, levels, GameMode, Theme, Powerup, PowerupManager, audioCues)
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu, PauseOverlay, ResultOverlay
...
- `SceneKeys` — centralizes scene key strings (`Boot`, `Preload`, `ThemeSelect`, `ModeSelect`, `Main`).
...
- `Theme.ts` — DXB-15: DX-Ball's visual-identity vocabulary (`neon-arcade` | `space` | `laboratory`), labels, backdrop/HUD/brick/powerup/overlay palettes, and load/save helpers. Read by ThemeSelect / ModeSelect / MainScene. `BrickGrid` / `Ball` / `Paddle` never import it.
...
- `SelectMenu` — DXB-14: reusable vertical option list (arrow keys, Space / Enter, click). `ModeSelectScene` is the first caller. Since DXB-13A: `destroy()` unbinds keys; configurable `depth`. Since DXB-15: `initialIndex` and `onHighlight` for live theme preview. Not DX-Ball-specific.
...
DXB-15 Theme & Visual Identity System — three selectable themes (Neon
Arcade, Space, Laboratory) recolor the backdrop, HUD, bricks, and
powerup capsules; every powerup has a unique icon with the letter as a
secondary cue; pause / victory / game-over use framed themed cards.
Gameplay systems are unchanged. See `docs/progress/DXB-15.md` for full
...
game modes, a pause menu, and a theme system (win, lose, score, best
score, lives, levels, powerups, SFX, brick types, readable
presentation, Classic / Time Attack / Endless, ESC overlay, selectable
visual identity). Good candidates going forward, none yet confirmed
with a requester:
- A live-playtested balance pass covering the new 7-type mix (drop
  ...
  Endless's speed ramp, and DXB-15's slightly larger capsules — the
  standing recommendation since DXB-06A/DXB-07/DXB-08, still using
  manual verification this session (see DXB-15's own Known Risks).
