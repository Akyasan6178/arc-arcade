...
src/
  scenes/     BootScene -> PreloadScene -> HubScene
              Play: ThemeSelectScene -> ModeSelectScene -> (Classic: LevelSelectScene preview) -> MainScene
              (GarageScene, StatsScene, AchievementsScene, SettingsScene,
              TutorialScene, CreditsScene, ReleaseNotesScene are Hub side screens)
  systems/    GameViewport, SceneKeys, HighScoreStore, AudioManager, ThemeStore, JsonStore
  entities/   dx-ball/ (Paddle, Ball, Brick, BrickType, BrickGrid, levels, GameMode, Theme, Progress, Leaderboards, LeaderboardAdapter, PlayerProfile, Skins, Powerup, PowerupManager, PowerupDropTable, LaserBolt, BrickImpactFx, RunSummary, paddleCosmetic, Version, ReleaseNotes, audioCues)
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu, PauseOverlay, ResultOverlay, ProgressList, StatsList, CollectionPreview, CatchFlash, PowerupCelebrate, TextButton, TabBar, LevelBrowser, TutorialStage, AudioPanel, ProfilePanel, NamePrompt, menuLayout
...
- `HubScene` — DXB-18A: visible main menu. Since DXB-26: compact card rows (Play / How To / Garage / What's New / Statistics / Achievements / Settings / Credits), version in the subtitle (`DX-Ball v0.28.0` since DXB-28), and creator/studio attribution in the hint. Play starts `ThemeSelectScene`. Optional G / S / U shortcuts still open Garage / Stats / Achievements. No gameplay.
...
- `SceneKeys` — centralizes scene key strings (`Boot`, `Preload`, `Hub`, `ThemeSelect`, `ModeSelect`, `LevelSelect`, `Tutorial`, `Credits`, `ReleaseNotes`, `Achievements`, `Stats`, `Garage`, `Settings`, `Main`).
...
- `Leaderboards.ts` — DXB-17: local Top 10 scores per mode (`classic` / `time-attack` / `endless`). Separate `dx-ball-leaderboards` key. `MainScene` submits a run once; `StatsScene` reads the lists. Since DXB-24: persistence is `LocalLeaderboardAdapter`; `OnlineLeaderboardAdapter` is an unwired future seam (no network, accounts, or cloud). Since DXB-28: each entry stores player name, score, mode, highest level reached, date, and version. `submitScore` returns rank / new-record for the end-of-run card. Gameplay does not change.
- `PlayerProfile.ts` — DXB-28: local display name (default `Player`, max 16 chars) under `dx-ball-player-profile`. Settings edits it; leaderboard submit copies it onto new entries. Not an account.
- `Skins.ts` — DXB-16: paddle / ball cosmetic tokens. `Paddle` / `Ball` never import it; the owning scene applies visuals. Since DXB-19: Crystal / Titan / Pulse / Obsidian paddles and Ice Core / Dark Matter / Solar / Nova balls. Since DXB-22: ball tokens also name an idle `fx` for animation; no new skin ids. Since DXB-23: paddle motifs still use these ids; unique silhouettes live in `paddleCosmetic.ts`. Since DXB-27: `getPaddleMotionHint()` names the motion identity for Garage.
- `Powerup` — DXB-09: a single falling capsule; a `Container` holding a colored rounded-rect background plus a one-letter label, one fixed color+letter pair per `PowerupType`. Purely visual/motion — all "what happens when caught" logic lives in `PowerupManager`. Since DXB-12: the union and visual map also include `fire-ball` (`F`), `multi-ball` (`M`), `small-paddle` (`N`), `fast-ball` (`T`) alongside the original `W`/`S`/`L`. Since DXB-13: positives use a green family and negatives a red/orange family; letters are unchanged. Since DXB-15: each type has a unique Graphics icon (heart, flame, spheres, expand, shrink, slow, fast); the letter is a small secondary cue; colors come from the active theme palette. Since DXB-24: `laser-paddle` (`Z`, dual-barrel icon) joins the roster.
- `PowerupManager` — DXB-09: owns every currently-falling `Powerup` for one run. `spawn(x, y)` picks a random configured type; `update()` advances each capsule and checks it against the paddle (caught → queued, no penalty either way if missed) or the bottom edge (removed); `consumeCaughtPowerups()` drains the caught queue for `MainScene` to react to. Deliberately never holds a `Ball` reference (which is replaced, not mutated, on every level transition) or touches lives/score directly. Since DXB-10: `spawn()` plays `'powerup-spawn'` and a catch plays `'powerup-collect'`. Since DXB-12: the default spawn pool is the full 7-type roster (positives and negatives share the existing drop pipeline); this manager still does not know what any type does. Since DXB-15: optional theme `palette` is forwarded into each capsule; spawn/catch/audio are unchanged. Since DXB-23: type picking uses `PowerupDropTable.ts` (weighted) instead of a uniform draw. Since DXB-24: Laser Paddle joins the pool. Since DXB-25: Extra Life 2% extremely rare, Laser 4% very rare, Fire Ball 8% rare; the manager still does not know what any type does.
- `PowerupDropTable.ts` — DXB-23: integer weights and `pickWeightedPowerupType()`. BrickGrid still only decides whether a capsule drops. Since DXB-25: Extra Life 2% extremely rare, Laser 4% very rare, Fire Ball 8% rare; weights still unique and still sum to 100.
- `LaserBolt.ts` — DXB-24: upward laser projectile. Visual + motion; overlap lives on `BrickGrid.resolveProjectileCollision()`.
- `BrickImpactFx.ts` — DXB-24: one-shot type-specific hit burst spawned by `BrickGrid`.
- `RunSummary.ts` — DXB-24: end-of-run copy (score / best / records / unlocks) for Classic / Endless / Time Attack cards. Since DXB-28: shareable format (Score, Mode, Highest Level Reached, Active Theme, Date) using the same labels as Statistics and Leaderboards.
- `LeaderboardAdapter.ts` — DXB-24: local Top 10 adapter plus an unwired online stub. DXB-28 expands the reusable `LeaderboardEntry` model. `Leaderboards.ts` is the public facade.
- `paddleCosmetic.ts` — DXB-23: shared paddle-body drawing for gameplay and Garage. Collision stays on `Paddle`. Since DXB-27: Robot side pistons pump with traveling caps; Alien emits expanding upward signal waves from an organic hull; Reactor has a spinning core with orbiting beads; Pulse carries one energy slug left to right.
- `Version.ts` — DXB-23: product version for Hub / Credits. Since DXB-28: `DX-Ball v0.28.0` (`0.<DXB-task>.0`) plus creator / studio attribution (`Haydar Talha Akyasan` / `Marka Mutfağı`).
- `ReleaseNotes.ts` — DXB-26: player-facing changelog cards for the What's New screen. Not git history. Since DXB-28: adds the Leaderboard Prep card.
- `audioCues.ts` — DXB-10: DX-Ball's own 8-event sound-effect vocabulary (`DxBallSfxKey` + synthesized `ToneSpec` per key) and the single `playDxBallSfx(key)` helper every entity/scene uses. DXB-22 also owns theme music keys / synthesized `MusicLoopSpec` beds and `playDxBallThemeMusic(themeId)`. DXB-24 adds `laser-fire`, `brick-hit`, and `powerup-celebrate`. Kept out of `AudioManager` the same way `levels.ts` keeps level data out of `BrickGrid`.
...
- `PauseOverlay` — DXB-13A: reusable pause/menu overlay (dim + title + actions). `MainScene` opens it on ESC or a visible Pause button (DXB-20). Since DXB-15: framed panel, accent bar, themed menu. Since DXB-26: visible Music / SFX on-off and volume meters via `AudioPanel`. Not DX-Ball-specific.
- `ResultOverlay` — DXB-15: reusable end/transition card (dim + framed panel + title + body). `MainScene` uses it for victory, game over, time-up, and level-clear. Since DXB-20: tap-to-continue plus a hint line. Since DXB-22: kicker + reward line and theme-colored panel glow. Since DXB-24: taller card so end-of-run copy can list score, best, records, and unlock progress. Since DXB-28: taller again for the shareable run summary. Not DX-Ball-specific.
- `CatchFlash` — DXB-22: one-shot full-viewport color flash for collection feedback. Since DXB-24: optional `celebrate` flash. Not DX-Ball-specific.
- `PowerupCelebrate` — DXB-24: strong-catch burst (label + ring) at the paddle. Not a gameplay system.
- `CollectionPreview` — DXB-18: live Garage preview. DXB-23 reuses `paddleCosmetic.ts` for paddle silhouettes. DXB-27 enlarges the paddle, drifts it, and names its motion identity.
- `TextButton` — DXB-18A: reusable tappable label. Visible Back / Favorite / Settings / Pause controls.
- `TabBar` — DXB-18A: reusable horizontal tab strip (tap or Left / Right). Garage and Statistics are the first callers. Since DXB-28: optional `bindKeyboard: false` for nested Local / Online tabs.
- `menuLayout` — DXB-20: shared menu chrome tokens (title / subtitle / hint / Back placement). Hub and every side screen read these so spacing stays consistent. Since DXB-23: accent rule and version caption helpers. Since DXB-28: sub-tab row for leaderboard Local / Online.
- `SelectMenu` — DXB-14: reusable vertical option list. Since DXB-26: card chrome and visual highlight instead of `> title <` debug prefixes.
- `LevelBrowser` — DXB-23: Classic campaign thumbnail grid. Since DXB-26: optional `previewOnly` so confirm inspects instead of starting a run.
- `TutorialStage` — DXB-27: full-width live how-to stage (looping miniature gameplay per page). TutorialScene is the caller. Replaces the DXB-26 three-column `TutorialBoard`.
- `AudioPanel` — DXB-26: shared Music / SFX toggles and volume meters. Pause and Settings are the callers.
- `ProfilePanel` — DXB-28: Settings card for the local player name. SettingsScene owns persistence.
- `NamePrompt` — DXB-28: DOM overlay for typing a short display name. Not a network form.
- `ProgressList` — DXB-16: reusable catalog list. Since DXB-26: completed achievement cards use gold fill, a large ✓, and a filled gold bar.
...
- `docs/progress/DXB-24.md`
- `docs/progress/DXB-25.md`
- `docs/progress/DXB-26.md`
- `docs/progress/DXB-27.md`
- `docs/progress/DXB-28.md`
...
DXB-28 Online Leaderboard Preparation. Local Top 10 still saves on
this device. Leaderboard entries now carry player name, score, mode,
highest level reached, date, and version. Settings stores a local
player name (default `Player`). End-of-run cards show a shareable
summary. Statistics Leaderboards have Local / Online (Coming Soon)
tabs; Online does not fabricate ranks. No accounts, networking, or
cloud. Product version is `DX-Ball v0.28.0`. See
`docs/progress/DXB-28.md`. `npm run typecheck` and `npm run build`
...
- A "+N" popup — collection flash and brick impacts are now in.
- Wiring `OnlineLeaderboardAdapter` to a real host (architecture only
  today).
