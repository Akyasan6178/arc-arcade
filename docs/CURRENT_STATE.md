...
src/
  ...
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu, PauseOverlay, ResultOverlay, ProgressList, StatsList, CollectionPreview, CatchFlash, TextButton, TabBar, menuLayout
...
- `Skins.ts` — DXB-16: paddle / ball cosmetic tokens. `Paddle` / `Ball` never import it; the owning scene applies visuals. Since DXB-19: Crystal / Titan / Pulse / Obsidian paddles and Ice Core / Dark Matter / Solar / Nova balls. Since DXB-22: ball tokens also name an idle `fx` for animation; no new skin ids.
...
DXB-22 Audio & Cosmetic Pass — presentation only. Theme background
music on the existing `AudioManager` seam (synthesized fallbacks;
mute still global; music volume is a separate internal bus). Existing
paddle / ball cosmetics animate with Phaser Graphics (Fire Ball still
overrides). Theme backdrops tick a lightweight particle overlay.
Powerup catches flash a short color overlay. Result cards and Garage
previews show those effects more clearly. No new gameplay, themes,
skins, achievements, or levels. See `docs/progress/DXB-22.md` for
audio / cosmetic / performance notes. `npm run typecheck` and
