...
src/
  ...
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground
...
DXB-13 Visual Refresh — brick types now have distinct drawings (clean
normal, cracked damage lines, metallic bands, highlighted bonus), Fire
Ball shows a hot fill plus glow, powerup capsules use green vs
red/orange polarity (letters kept), the HUD shares one bold
typeface/stroke language, and a lightweight `ArcadeBackground` sits
behind the playfield. Score, lives, levels, powerups, and audio
behavior are unchanged. See `docs/progress/DXB-13.md` for full details.
...
powerup roster, audio, advanced brick types, and a visual refresh (win,
lose, score, best score, lives, levels, powerups, SFX, brick types,
readable presentation). Good candidates going forward, none yet
...
- A live-playtested balance pass covering the new 7-type mix (drop
  ...
  verification this session (see DXB-13's own Known Risks).
