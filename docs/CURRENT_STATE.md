...
src/
  ...
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu, PauseOverlay, ResultOverlay, ProgressList, StatsList, CollectionPreview, CatchFlash, PowerupCelebrate, TextButton, TabBar, LevelBrowser, TutorialBoard, AudioPanel, menuLayout
...
- `HubScene` — DXB-18A: visible main menu. Since DXB-26: compact card rows (Play / How To / Garage / What's New / Statistics / Achievements / Settings / Credits), `DX-Ball v0.26.0` in the subtitle, and creator/studio attribution in the hint. Play starts `ThemeSelectScene`. Optional G / S / U shortcuts still open Garage / Stats / Achievements. No gameplay.
...
- `TutorialScene` — DXB-26: Hub how-to rebuilt as five visual pages (Controls / Bricks / Powerups / Modes / Progression). Center is a live gameplay vignette; sides are short labels. Visible Previous / Next / Back. Esc still returns to the Hub. No gameplay.
- `CreditsScene` — DXB-26: one compact card — DX-Ball, Created By Haydar Talha Akyasan, Powered By Marka Mutfağı, Version v0.26.0. Esc / Back return to the Hub. No gameplay.
...
- `paddleCosmetic.ts` — DXB-23: shared paddle-body drawing for gameplay and Garage. Collision stays on `Paddle`. Since DXB-25: robot side pistons, alien signal waves above the hull, a rotating reactor core, and traveling pulse bands.
- `Version.ts` — DXB-23: product version for Hub / Credits. Since DXB-26: `DX-Ball v0.26.0` (`0.<DXB-task>.0`) plus creator / studio attribution (`Haydar Talha Akyasan` / `Marka Mutfağı`).
- `ReleaseNotes.ts` — DXB-26: player-facing changelog cards for the What's New screen. Not git history.
...
- `TutorialBoard` — DXB-26: visual-first how-to board (center vignette, short notes left/right, portrait stacks). TutorialScene is the caller.
...
DXB-26 UI & Presentation Correction Pass. Classic always starts at
Level 1; the Level Browser is preview-only. Time Attack gives every
level a fresh 90s clock and clears timed effects. Pause exposes Music /
SFX on-off and volume. How To is a five-page visual board. Credits name
Haydar Talha Akyasan and Marka Mutfağı on one compact card. Product
version is `DX-Ball v0.26.0`. Hub has a What's New changelog. Card
menus replace debug `> title <` lists. Completed achievements use a
large gold ✓. No new modes, themes, achievements, currencies, or online
systems. See `docs/progress/DXB-26.md`. `npm run typecheck` and
