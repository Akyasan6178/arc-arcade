...
src/
  ...
  entities/   dx-ball/ (Paddle, Ball, Brick, BrickType, BrickGrid, levels, GameMode, Theme, Progress, Leaderboards, LeaderboardAdapter, Skins, Powerup, PowerupManager, PowerupDropTable, LaserBolt, BrickImpactFx, RunSummary, paddleCosmetic, Version, ReleaseNotes, audioCues)
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu, PauseOverlay, ResultOverlay, ProgressList, StatsList, CollectionPreview, CatchFlash, PowerupCelebrate, TextButton, TabBar, LevelBrowser, TutorialStage, AudioPanel, menuLayout
...
- `HubScene` — DXB-18A: visible main menu. Since DXB-26: compact card rows (Play / How To / Garage / What's New / Statistics / Achievements / Settings / Credits), version in the subtitle (`DX-Ball v0.27.0` since DXB-27), and creator/studio attribution in the hint. Play starts `ThemeSelectScene`. Optional G / S / U shortcuts still open Garage / Stats / Achievements. No gameplay.
...
- `CreditsScene` — DXB-26: one compact card — DX-Ball, Created By Haydar Talha Akyasan, Powered By Marka Mutfağı, Version from `Version.ts` (`v0.27.0` since DXB-27). Esc / Back return to the Hub. No gameplay.
...
- `RunSummary.ts` — DXB-24: end-of-run copy (score / best / records / unlocks) for Classic / Endless / Time Attack cards.
- `LeaderboardAdapter.ts` — DXB-24: local Top 10 adapter plus an unwired online stub. `Leaderboards.ts` is the public facade.
...
- `Version.ts` — DXB-23: product version for Hub / Credits. Since DXB-27: `DX-Ball v0.27.0` (`0.<DXB-task>.0`) plus creator / studio attribution (`Haydar Talha Akyasan` / `Marka Mutfağı`).
- `ReleaseNotes.ts` — DXB-26: player-facing changelog cards for the What's New screen. Not git history. Since DXB-27: adds the How To & Paddles card.
...
DXB-27 Tutorial & Paddle Redesign Pass. How To is a five-page live
showcase instead of a three-column reading board. Robot / Alien /
Reactor / Pulse paddles use obvious motion (pistons, upward waves,
orbiting core, traveling energy slug). Garage previews name that
motion. Product version is `DX-Ball v0.27.0`. No new modes, themes,
achievements, currencies, or online systems. See
`docs/progress/DXB-27.md`. `npm run typecheck` and `npm run build`
