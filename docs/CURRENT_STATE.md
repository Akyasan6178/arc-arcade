...
src/
  ...
  entities/   dx-ball/ (Paddle, Ball, Brick, BrickType, BrickGrid, levels, GameMode, Theme, Progress, Leaderboards, LeaderboardAdapter, Skins, Powerup, PowerupManager, PowerupDropTable, LaserBolt, BrickImpactFx, RunSummary, paddleCosmetic, Version, audioCues)
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu, PauseOverlay, ResultOverlay, ProgressList, StatsList, CollectionPreview, CatchFlash, PowerupCelebrate, TextButton, TabBar, LevelBrowser, TutorialBoard, menuLayout
...
- `LevelSelectScene` — DXB-23: Classic campaign browser. Miniature brick-map thumbnails, level number, and name for all 10 campaign entries. Since DXB-24: brick-type icons and a 1–5 difficulty rating on each card. Since DXB-25: inner thumbnail well, colored difficulty pips, and type-letter chips. Confirm starts `MainScene` with `{ mode: 'classic', startLevelIndex }`. Esc / Back return to Mode Select. No gameplay.
- `TutorialScene` — DXB-23: Hub how-to (Paddle / Bricks / Drops / Modes tabs). Since DXB-25: visual three-column board (gameplay example center, short notes left/right) via `TutorialBoard`. Esc / Back return to the Hub. No gameplay.
- `CreditsScene` — DXB-23: title and development credits. Since DXB-25: card layout for Powered By (Marka Mutfağı), Created By (Akif Yasan), and `DX-Ball v0.25.0`. Esc / Back return to the Hub. No gameplay.
- `AchievementsScene` — DXB-18A: dedicated achievements list (the catalog that lived inside Unlockables in DXB-16). Locked / unlocked / percent rows; no equip. Since DXB-25: completed rows use gold chrome, a ribbon, and a completion badge. Visible Back and Esc return to the Hub (or ThemeSelect / ModeSelect). No gameplay.
...
- `Version.ts` — DXB-23: product version for Hub / Credits. Since DXB-25: `DX-Ball v0.25.0` (`0.<DXB-task>.0`) plus creator / studio attribution (`Akif Yasan` / `Marka Mutfağı`).
...
- `LevelBrowser` — DXB-23: Classic campaign thumbnail grid. Since DXB-24: type icons and a difficulty rating under the miniature layout. Since DXB-25: inner thumbnail well, colored difficulty pips, and type-letter chips. Not a gameplay system.
- `TutorialBoard` — DXB-25: visual how-to board (center example, short notes left/right). TutorialScene is the caller. Not a gameplay system.
- `ProgressList` — DXB-16: reusable catalog list. Since DXB-25: optional `completeChrome` (gold card, ribbon, completion badge) for Achievements.
...
DXB-25 Player Experience Pass. Fire Ball and Laser Paddle last 5s.
Final weighted table: Extra Life 2% extremely rare, Laser 4% very rare,
Fire Ball 8% rare, Multi Ball 13% uncommon, commons the rest. Time
Attack clears timed powerups on level wrap (score stays). Paddle
cosmetics have stronger silhouettes. Completed achievements use gold
chrome. Tutorial is a visual three-column board. Credits name Marka
Mutfağı and Akif Yasan. Product version is `DX-Ball v0.25.0`. Settings
expose mute plus music / SFX volume. Level cards show thumbnail,
name, difficulty pips, and brick-type chips. No new modes, themes,
achievements, currencies, or online systems. See
`docs/progress/DXB-25.md`. `npm run typecheck` and `npm run build`
