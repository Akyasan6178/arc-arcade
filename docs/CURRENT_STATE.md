...
- `Skins.ts` — DXB-16: paddle / ball cosmetic tokens. `Paddle` / `Ball` never import it; the owning scene applies visuals. Since DXB-19: Crystal / Titan / Pulse / Obsidian paddles and Ice Core / Dark Matter / Solar / Nova balls.
...
DXB-21 Balance Pass — playtest tuning only. Fire Ball duration 10s →
7s (metal still one-hit on fire; smallest change vs rewriting
`takeHit()`). Multi Ball extras split ±20° → ±10° (cap still 3). Time
Attack applies a 1.15× speed fold through the existing progression
multiplier; the clock stays 90 seconds. Endless ramps at +0.15%/s
instead of +0.4%/s so the 2× cap lands around 11 minutes instead of
~4, removing the level-4 wall while leaving level 20 as a skill
ceiling. No new systems, content, themes, or achievements. See
`docs/progress/DXB-21.md` for playtest findings, final values, and
rationale. `npm run typecheck` and `npm run build` both verified
passing.
