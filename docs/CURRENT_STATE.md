...
src/
  scenes/     BootScene -> PreloadScene -> HubScene
              Play: ThemeSelectScene -> ModeSelectScene -> MainScene
...
- `SceneKeys` — centralizes scene key strings (`Boot`, `Preload`, `Hub`, `ThemeSelect`, `ModeSelect`, `Achievements`, `Stats`, `Garage`, `Settings`, `Main`).
...
DXB-18A UI Navigation Refactor — a visible Hub (Play / Garage /
Statistics / Achievements / Settings) after preload, tabbed Garage and
Statistics, a dedicated Achievements screen, a Settings screen that
exposes the existing mute toggle, and visible Back controls so every
screen is reachable without keyboard shortcuts. G / S / U remain
optional. Unlocks, achievements, themes, modes, stats, leaderboards,
and gameplay are unchanged. See `docs/progress/DXB-18A.md` for full
...
stats / leaderboards, a garage / collection hub, and a menu-driven
Hub so those screens are reachable on mobile (win, lose, score,
...
equip / favorites, visible navigation). Good candidates going
