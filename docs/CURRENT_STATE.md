...
src/
  ...
  ui/         ScoreLabel, ActiveEffectsLabel, ArcadeBackground, ModeLabel, SelectMenu, PauseOverlay, ResultOverlay, ProgressList, StatsList, CollectionPreview, CatchFlash, PowerupCelebrate, TextButton, TabBar, LevelBrowser, menuLayout
...
- `LevelSelectScene` — DXB-23: Classic campaign browser. Miniature brick-map thumbnails, level number, and name for all 10 campaign entries. Since DXB-24: brick-type icons and a 1–5 difficulty rating on each card. Confirm starts `MainScene` with `{ mode: 'classic', startLevelIndex }`. Esc / Back return to Mode Select. No gameplay.
...
- `PowerupManager` — DXB-09: owns every currently-falling `Powerup` for one run. `spawn(x, y)` picks a random configured type; `update()` advances each capsule and checks it against the paddle (caught → queued, no penalty either way if missed) or the bottom edge (removed); `consumeCaughtPowerups()` drains the caught queue for `MainScene` to react to. Deliberately never holds a `Ball` reference (which is replaced, not mutated, on every level transition) or touches lives/score directly. Since DXB-10: `spawn()` plays `'powerup-spawn'` and a catch plays `'powerup-collect'`. Since DXB-12: the default spawn pool is the full 7-type roster (positives and negatives share the existing drop pipeline); this manager still does not know what any type does. Since DXB-15: optional theme `palette` is forwarded into each capsule; spawn/catch/audio are unchanged. Since DXB-23: type picking uses `PowerupDropTable.ts` (weighted; Extra Life 3%, Fire Ball 8%) instead of a uniform draw. Since DXB-24: Laser Paddle joins the pool (6% rare); the manager still does not know what any type does.
- `PowerupDropTable.ts` — DXB-23: integer weights and `pickWeightedPowerupType()`. BrickGrid still only decides whether a capsule drops. Since DXB-24: Laser Paddle 6%; weights still unique and still sum to 100.
...
- `LevelBrowser` — DXB-23: Classic campaign thumbnail grid. Since DXB-24: type icons and a difficulty rating under the miniature layout. Not a gameplay system.
...
DXB-24 Fun Factor Update. Laser Paddle is a rare 10s powerup (dual
upward bolts; metal takes 3 laser hits). Brick hits have type-specific
flash/debris/sparks. Fire Ball / Multi Ball / Laser Paddle / Extra Life
celebrate on catch. Classic level cards show mini layout, type icons,
and a 1–5 rating. End-of-run cards list score, best, records, and
unlock progress. Leaderboards stay local, with an unwired online
adapter seam. No currency, purchases, accounts, or cloud saves. See
`docs/progress/DXB-24.md`. `npm run typecheck` and `npm run build`
